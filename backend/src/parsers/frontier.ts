...
export function parseFrontier(subject: string, body: string): ParserResult {
  const fullText = `${subject}\n${body}`;

  // Frontier confirmation number
  const confirmMatch = fullText.match(/confirmation\s*(?:code|number|#)?[:\s]*([A-Z0-9]{6})/i);
  const confirmation = confirmMatch ? confirmMatch[1] : undefined;

  // Frontier route patterns
